    ...
    public class BoundlessDbContext
    {
        ...
        public List<Transaction> GetTransactions()
        {
            ...
            using (MySqlConnection connection = GetConnection())
            {
                ...
                MySqlCommand query = new MySqlCommand(
                    "SELECT transactions.TransactionID, transactions.DATE, items.ItemName, transactions.Coins, transactions.UnitaryValue, transactions.Quantity, locations.NAME " +
                    "FROM transactions INNER JOIN items ON items.ItemId = transactions.ItemId INNER JOIN locations ON locations.LocationID = transactions.LocationId " +
                    "ORDER BY transactions.DATE DESC, transactions.TransactionID DESC", connection);
        ...
        public bool UpdateTransaction(Transaction transaction)
        {
            ...
            try
            {
                ...
                string sqlcmd = $"UPDATE transactions SET " +
                    $"ItemId = {itemId}," +
                    $"Date = '{transaction.Date.Year}-{transaction.Date.Month}-{transaction.Date.Day}'," +
                    $"Coins = {coins}," +
                    $"UnitaryValue = {transaction.UnitaryValue}," +
                    $"Quantity = {quantity}," +
                    $"LocationId = {locationId} " +
                    $"WHERE transactions.TransactionId = {transaction.TransactionId};";
